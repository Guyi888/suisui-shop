...
if($type=='alipay'&&$conf['alipay_api']==5 || $type=='qqpay'&&$conf['qqpay_api']==5 || $type=='wxpay'&&$conf['wxpay_api']==5){ //码支付
	...
	// 如果有redirect_url，保存到session
	if(!empty($redirect_url)){
		session_start();
		$_SESSION['pay_redirect_url'] = $redirect_url;
	}
	echo "<script>window.location.href='./codepay.php?type={$type}&trade_no={$orderid}';</script>";

}elseif($type=='alipay'&&$conf['alipay_api']==2 || $type=='qqpay'&&$conf['qqpay_api']==2 || $type=='wxpay'&&$conf['wxpay_api']==2 || $type=='qqpay'&&$conf['qqpay_api']==8 || $type=='wxpay'&&$conf['wxpay_api']==8 || $type=='wxpay'&&$conf['wxpay_api']==9 || $type=='alipay'&&$conf['alipay_api']==10 || $type=='qqpay'&&$conf['qqpay_api']==10 || $type=='wxpay'&&$conf['wxpay_api']==10){ //易支付
