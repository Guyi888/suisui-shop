...
class AlipayTradeService {
	...
	public function orderQuery($params) {
		...
		if (is_array($params)) {
			if (isset($params['out_trade_no'])) {
				$queryContentBuilder->setOutTradeNo($params['out_trade_no']);
			} elseif (isset($params['trade_no'])) {
				$queryContentBuilder->setTradeNo($params['trade_no']);
			} else {
				throw new Exception('缺少必要的订单参数');
			}
		} else {
	...
	public function check($arr){
		...
		fwrite($log, "\n\n" . str_repeat('=', 50) . "\n");
		fwrite($log, '签名验证开始: ' . date('Y-m-d H:i:s') . "\n");
		...
		if($result){
			...
			fwrite($log, '订单查询结果: ' . json_encode($queryResponse) . "\n");
