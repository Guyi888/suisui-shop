...
function getPoint() {
	...
	var isinvitegift = $('#tid option:selected').attr('isinvitegift');
	var invitegift_money = $('#tid option:selected').attr('invitegift_money');
	var invite_gift = $('#tid option:selected').attr('invite_gift');
...
function getshop(tid,cid,name,price,input,inputs,multi,isfaka,isinvitegift,invitegift_money,invite_gift,value,desc,alert,shopimg,close,prices,max,min,stock){
	...
	$("#imgshop").html('<img src="'+shopimg+'" onerror="this.src=\'/assets/img/Product/default.png\'">');
	if(isfaka==1){
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				layer.open({
					...
					zIndex: 2001,
					content: item,
					closeBtn: '<a href="javascript:;" class="layui-layer-close layui-layer-close1" style="font-size: 20px; color: #666;">X</a>',
					shadeClose: true
				});
			}else{
				layer.alert(data.msg);
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				$.each(data.data, function (i, res) {
					$("#shoplist").append('<div class="col-xs-6 col-sm-3 col-md-3 layui-anim layui-anim-scaleSpring" data-anim="layui-anim-upbit"><a href="javascript:void(0)" id="'+res.tid+'" onclick="getshop('+res.tid+',\''+cid+'\',\''+res.name+'\',\''+res.price+'\',\''+res.input+'\',\''+res.inputs+'\',\''+res.multi+'\',\''+res.isfaka+'\',\''+res.isinvitegift+'\',\''+res.invitegift_money+'\',\''+res.invite_gift+'\',\''+res.value+'\',\''+escape(res.desc)+'\',\''+escape(res.alert)+'\',\''+res.shopimg+'\',\''+res.close+'\',\''+res.prices+'\',\''+res.max+'\',\''+res.min+'\',\''+res.stock+'\');" value="'+res.tid+'"><div class="thumbnail" style="height:240px;"><center style="margin-top:5%;"><img src="'+res.shopimg+'" width="80" height="100" style="border-radius: 15px" onerror="this.src=\'/assets/img/Product/default.png\'"><hr class="layui-bg-blue" style="width:100%">'+res.name+'<hr class="layui-bg-red" style="width:100%">[￥'+res.price+']<br>'+(res.close==1?'<span class="layui-badge layui-bg-yellow">停止下单</span>':'<span class="layui-badge layui-bg-blue">立即购买</span>')+'</center></div></a></div>');
					num++;
				});
				$("#shoplist").val(0);
				/*getPoint();*/
