	...
	$.ajax({
		...
		success : function(data) {
			...
			item += '</td></tr>';
				...
				if(data.desc){
					item += '<tr><td colspan="6" style="text-align:center" class="orderTitle"><b>商品简介</b>'+(data.ts?'<a href="./workorder2.php?my=add&orderid='+id+'&skey='+skey+'&ts=1" target="_blank" onclick="return checklogin('+data.islogin+')" class="btn btn-xs btn-danger">网盘链接失效投诉</a>':'')+'</td><tr><td colspan="6" class="orderContent">'+data.desc+'</td></tr>';
				}
				item += '</table>';
