...
function display_djzt($zt, $id = 0)
{
	if ($zt == 1) {
		...
		return "<span onclick=\"djOrder(" . $id . ")\" title=\"点击重试\" class=\"btn btn-danger btn-xs resubmit\" data-id=\"" . $id . "\">失败</span> <span onclick=\"manualFaka(" . $id . ")\" title=\"手动输入卡密发卡\" class=\"btn btn-warning btn-xs\">手动发卡</span>";
	} elseif ($zt == 3) {
		return "<a onclick=\"window.open('fakakms.php?orderid=" . $id . "')\" title=\"查看卡密信息\"><font color=green>已发卡</font></a>";
	} elseif ($zt == 4) {
		return "<span onclick=\"djOrder(" . $id . ")\" title=\"点击重试\" class=\"btn btn-danger btn-xs\">未发卡</span> <span onclick=\"manualFaka(" . $id . ")\" title=\"手动输入卡密发卡\" class=\"btn btn-warning btn-xs\">手动发卡</span>";
	} else {
