...
if ($_GET["my"] == "setdo" && $_POST["do"] == "submit") {
	...
	if ($rank_reward > 0) {
		if ($rank_percentage == null) {
			showmsg("奖励销量的比例不能为空", 3);
		} elseif ($rank_percentage < 0 || $rank_percentage > 50) {
			showmsg("请设置合理的奖励比例", 3);
		}
	}
	...
	<tbody>
		<tr>
			<th style="font-size: 13px;" class="text-center">
				<i class="fa fa-calendar-times-o"></i> 上次发放奖励时间<br><?php echo $cron_lasttime;?>			</th>
			<th style="font-size: 13px;" class="text-center">
				<i class="fa fa-money"></i> 上次发放奖励金额<br><?php echo $cron_money;?>			</th>
		</tr>
	</tbody>
</table>
