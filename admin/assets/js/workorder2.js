...
function listTable(query){
	...
	if(query == 'start' || query == undefined){
		query = '';
		history.replaceState({}, null, './workorder2.php');
	}else if(query != undefined){
		history.replaceState({}, null, './workorder2.php?'+query);
	}
	layer.closeAll();
	var ii = layer.load(2, {shade:[0.1,'#fff']});
	$.ajax({
		type : 'GET',
		url : 'workorder2-table.php?'+query,
		dataType : 'html',
		cache : false,
...
function orderItem(id){
	...
	var url = './workorder2-item.php?my=view&id='+id;
