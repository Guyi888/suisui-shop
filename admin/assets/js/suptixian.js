function listTable(query){
	...
	if(query == 'start' || query == undefined){
		query = '';
		history.replaceState({}, null, './suptixian.php');
	}else if(query != undefined){
		history.replaceState({}, null, './suptixian.php?'+query);
	}
	layer.closeAll();
	var ii = layer.load(2, {shade:[0.1,'#fff']});
	$.ajax({
		type : 'GET',
		url : 'suptixian-table.php?'+query,
		dataType : 'html',
		cache : false,
...
function inputInfo(id) {
	...
	$.ajax({
		type : 'GET',
		url : 'ajax_sup.php?act=getsuptixian&id='+id,
		dataType : 'json',
		success : function(data) {
			layer.close(ii);
			if(data.code == 0){
...
function saveInfo(id) {
	...
	$.ajax({
		type : "POST",
		url : "ajax_sup.php?act=edittixian",
...
function skimg(sid){
	layer.open({
		...
		content: '<center><img width="300px" src="../assets/img/skimg/sk_sup'+sid+'.png"></center>'
	});
}
...
function setResult(id) {
	...
	$.ajax({
		type : 'POST',
		url : 'ajax_sup.php?act=tixian_note',
		data : {id:id},
		dataType : 'json',
		success : function(data) {
			layer.close(ii);
			if(data.code == 0){
				...
				$.ajax({
					type : 'POST',
					url : 'ajax_sup.php?act=set_suptixian_note',
