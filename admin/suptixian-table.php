...
$numrows = $DB->getColumn("SELECT count(*) from pre_suptixian WHERE" . $sql);
?>	<div class="table-responsive">
        <table class="table table-striped">
          <thead><tr><th><input type="checkbox" id="checkbox" value="" /></th><th>ID</th><th>SID</th><th>金额</th><th>实际到账</th><th>提现方式</th><th>提现账号</th><th>姓名</th><?php echo $conf["sup_skimg"] == 1 ? "<th>收款图</th>" : null;?><th>申请时间</th><th>完成时间</th><th>状态</th><th>信息</th><th>操作</th></tr></thead>
          <tbody>
<?php
$pagesize = 30;
...
$rs = $DB->query("SELECT * FROM pre_suptixian WHERE" . $sql . " ORDER BY id DESC limit " . $offset . "," . $pagesize);
while ($res = $rs->fetch()) {
	$addstr = "";
	if ($res["status"] == 0) {
		$addstr = "<a href=\"javascript:operation(" . $res["id"] . ",'complete')\" class=\"btn btn-success btn-xs\">完成</a>&nbsp;" . ($conf["sup_daifu"] > 0 && isset($_SESSION["transfer"]) ? "<a href=\"javascript:transfer(" . $res["id"] . ")\" class=\"btn btn-primary btn-xs transfer_do\">转账</a>&nbsp;" : null) . "<a href=\"javascript:setfail(" . $res["id"] . ",'" . $res["money"] . "')\" class=\"btn btn-xs btn-warning\">失败</a>&nbsp;";
	} elseif ($res["status"] == 1) {
		$addstr = "<a href=\"javascript:operation(" . $res["id"] . ",'reset')\" class=\"btn btn-info btn-xs\">撤销</a>&nbsp;";
	}
	echo "<tr data-id=\"" . $res["id"] . "\"><td width=\"10\" style=\"vertical-align:middle;\"><input type=\"checkbox\" id=\"" . $res["id"] . "\" value=\"\" name=\"data\" /></td><td><b>" . $res["id"] . "</b></td><td><a href=\"sitelist.php?sid=" . $res["sid"] . "\" target=\"_blank\">" . $res["sid"] . "</a></td><td>" . $res["money"] . "</td><td>" . $res["realmoney"] . "</td><td>" . display_type($res["pay_type"]) . "</td><td><span onclick=\"inputInfo(" . $res["id"] . ")\" title=\"修改信息\">" . $res["pay_account"] . "</span></td><td><span onclick=\"inputInfo(" . $res["id"] . ")\" title=\"修改信息\">" . $res["pay_name"] . "</span></td>" . ($conf["sup_skimg"] == 1 ? "<td><a href=\"javascript:skimg(" . $res["sid"] . ")\">点击查看</a></td>" : null) . "<td>" . $res["addtime"] . "</td><td>" . ($res["status"] == 1 ? $res["endtime"] : null) . "</td><td class=\"status\">" . display_zt($res["status"], $res["id"]) . "</td><td class=\"transfer_info\"></td><td class=\"op\">" . $addstr . "<a href=\"./suprecord.php?sid=" . $res["sid"] . "\" class=\"btn btn-default btn-xs\" target=\"_blank\">明细</a>&nbsp;<a href=\"javascript:delItem(" . $res["id"] . ")\" class=\"btn btn-xs btn-danger\">删除</a></td></tr>";
}
?>          </tbody>
        </table>
      </div>
