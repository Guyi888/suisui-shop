...
while ($res = $rs->fetch()) {
	...
	if (intval($res["prid"]) > 0) {
		...
		echo "<td data-label=\"价格\"><button type=\"button\" class=\"admin-shop-cell-button\" data-shop-action=\"price\" data-tid=\"" . intval($res["tid"]) . "\"><span class=\"admin-shop-price admin-shop-price--manual\"><span>零售 <strong>" . q8_admin_shop_table_escape($effectivePrices["price"]) . "</strong></span><span>普及 <strong>" . q8_admin_shop_table_escape($effectivePrices["cost"]) . "</strong></span><span>专业 <strong>" . q8_admin_shop_table_escape($effectivePrices["cost2"]) . "</strong></span></span></button></td>";
	}
