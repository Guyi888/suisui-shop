	...
	public function batch_goods_list(){
		...
		if (isset($json['code']) && $json['code']==1) {
			...
			foreach($json['data'] as $rows){
				...
				}elseif(isset($rows['groupaliasname'])){
					$class_name = $rows['groupaliasname'];
				}elseif(isset($rows['classname'])){
					$class_name = $rows['classname'];
				}elseif(isset($rows['name'])){
				...
				if(isset($rows['goods']) && is_array($rows['goods'])){
					foreach($rows['goods'] as $row){
						$list[] = array(
							...
							'name' => $row['goodsname'],
							'shopimg' => $row['imgurl'],
							'price' => round($row['goodsprice']/100, 2),
							...
							'original_cid' => $rows['id'],
							'original_cname' => $class_name
						);
					}
				}
			}
		...
		if (isset($json['code']) && $json['code']==1) {
			...
			foreach($json['data'] as $row){
				...
				if(isset($row['groupname'])){
					$class_name = $row['groupname'];
				}elseif(isset($row['groupaliasname'])){
					$class_name = $row['groupaliasname'];
				}elseif(isset($row['classname'])){
