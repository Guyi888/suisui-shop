...
<div class="wrapper">
	<div class="col-sm-12">
		...
		?>
				...
				<div class="panel-body">
					<form action="./uset.php?mod=user_n" method="post" role="form">
						...
						<?php if ($userrow['power'] > 0) { ?>
							<div class="form-group">
								<label>提现方式:</label><br />
								<select class="form-control" name="pay_type" default="<?php echo $userrow['pay_type'] ?>"><?php if ($conf['fenzhan_tixian_alipay'] == 1) { ?><option value="0">支付宝</option><?php }
																																																		if ($conf['fenzhan_tixian_wx'] == 1) { ?><option value="1">微信</option><?php }
																																																																			if ($conf['fenzhan_tixian_qq'] == 1) { ?><option value="2">QQ钱包</option><?php } ?></select>
							</div>
							<div class="form-group">
								<label>提现账号:</label><br />
			...
			<?php if (substr($userrow['user'], 0, 3) == 'qq_') { ?>
				<div class="panel panel-default">
					<div class="panel-heading font-bold">登录用户名与密码设置</div>
					<div class="panel-body">
						...
						<form onsubmit="return setpwd()" method="post" role="form">
							<div class="form-group">
								<label>登录用户名:</label><br />
								<input type="text" name="user" placeholder="输入登录用户名" class="form-control" required />
								<font color="green">登录用户名一经设置无法修改</font>
							</div>
							<div class="form-group">
								<label>登录密码:</label><br />
			...
			if ($sitename == null) {
				...
				if ($userrow['power'] == 2) {
					...
					if ($conf['fenzhan_edithtml'] == 1) {
							...
							$sql = "UPDATE `pre_site` SET `sitename`=:sitename,`title`=:title,`keywords`=:keywords,`description`=:description,`kfqq`=:kfqq,`kfwx`=:kfwx,`anounce`=:anounce,`modal`=:modal,`modal_show_type`=:modal_show_type,`bottom`=:bottom,`alert`=:alert,`ktfz_price`=:ktfz_price,`ktfz_price2`=:ktfz_price2,`ktfz_domain`=:ktfz_domain,`template`=:template,`appurl`=:appurl WHERE `zid`=:zid";
							$data = [':sitename' => $sitename, ':title' => $title, ':keywords' => $keywords, ':description' => $description, ':kfqq' => $kfqq, ':kfwx' => $kfwx, ':anounce' => $anounce, ':modal' => $modal, ':modal_show_type' => $modal_show_type, ':bottom' => $bottom, ':alert' => $alert, ':ktfz_price' => $ktfz_price, ':ktfz_price2' => $ktfz_price2, ':ktfz_domain' => $ktfz_domain, ':template' => $template, ':appurl' => $appurl, ':zid' => $zid];
						} else {
					...
					if ($conf['fenzhan_edithtml'] == 1) {
							$modal_show_type = isset($_POST['modal_show_type']) ? intval($_POST['modal_show_type']) : 0;
							$sql = "UPDATE `pre_site` SET `sitename`=:sitename,`title`=:title,`keywords`=:keywords,`description`=:description,`kfqq`=:kfqq,`kfwx`=:kfwx,`anounce`=:anounce,`modal`=:modal,`modal_show_type`=:modal_show_type,`bottom`=:bottom,`alert`=:alert,`template`=:template,`appurl`=:appurl WHERE `zid`=:zid";
							$data = [':sitename' => $sitename, ':title' => $title, ':keywords' => $keywords, ':description' => $description, ':kfqq' => $kfqq, ':kfwx' => $kfwx, ':anounce' => $anounce, ':modal' => $modal, ':modal_show_type' => $modal_show_type, ':bottom' => $bottom, ':alert' => $alert, ':template' => $template, ':appurl' => $appurl, ':zid' => $zid];
						} else {
		...
		?>
			<div class="panel panel-default">
				<div class="panel-heading font-bold">网站信息设置</div>
				<div class="panel-body">
					<form action="./uset.php?mod=site_n" method="post" role="form">
						...
						<?php if ($conf['fenzhan_edithtml'] == 1) { ?>
							...
							<div class="form-group">
								<label>首页弹出公告</label>
								<select class="form-control" name="modal_show_type" style="width:auto;display:inline;margin-left:10px;" default="<?php echo isset($userrow['modal_show_type']) ? $userrow['modal_show_type'] : '0';?>">
						...
						if ($userrow['power'] == 2) { ?>
							<div class="form-group">
								...
								<input type="text" name="ktfz_price" value="<?php echo $userrow['ktfz_price'] > 0 ? $userrow['ktfz_price'] : $conf['fenzhan_price']; ?>" class="form-control" />
								<pre>前台自助开通分站的价格</pre>
							</div>
							<div class="form-group">
								<label>专业分站价格</label><br>
								<input type="text" name="ktfz_price2" value="<?php echo $userrow['ktfz_price2'] > $conf['fenzhan_cost2'] ? $userrow['ktfz_price2'] : $conf['fenzhan_price2']; ?>" class="form-control" />
								<pre>前台自助开通分站的价格，不能低于成本价<?php echo $conf['fenzhan_cost2'] ?>元</pre>
							</div>
							<div class="form-group">
								<label>分站可选择域名</label><br>
								<input type="text" name="ktfz_domain" value="<?php echo $userrow['ktfz_domain']; ?>" class="form-control" />
						...
						<?php if ($conf['fenzhan_editd'] > 0) { ?>
							<div class="form-group">
								...
								<div class="input-group">
									<input type="text" name="domain" value="<?php echo $userrow['domain']; ?>" class="form-control" disabled />
		...
		<?php
		} elseif ($mod == 'site2' && $userrow['power'] > 0) {
			$mblist = \lib\Template::getList();
		?>
			<div class="panel panel-default">
				<div class="panel-heading font-bold">模板配置</div>
				<div class="panel-body">
					<form action="./uset.php?mod=site_n" method="post" role="form">
						<?php if ($conf['fenzhan_template'] == 1) { ?>
							<div class="form-group">
...
<div class="panel-heading font-bold">更改首页LOGO</div>
...
<div class="panel-body">';
			...
			echo '<div class="panel panel-default">
<div class="panel-heading font-bold">客服微信二维码设置</div>
<div class="panel-body">';
			if ($_POST['s'] == 1) {
				if (!checkRefererHost()) exit();
				// 检查文件类型和大小
